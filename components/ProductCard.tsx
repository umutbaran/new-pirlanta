...
import { Product } from '@/data/products'; // Type definition için kalabilir veya db'den alabiliriz
...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/urun/${product.id}`} className="group block bg-white hover:shadow-2xl transition-all duration-500 rounded-sm overflow-hidden border border-transparent hover:border-gray-100 h-full flex flex-col">
      <div className="relative aspect-square overflow-hidden bg-gray-50">
        ...
        {/* Buton */}
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 translate-y-12 group-hover:translate-y-0 transition-transform duration-300 z-20 w-full px-4">
            <span className="block w-full bg-white text-black text-xs font-bold uppercase tracking-widest py-3 text-center shadow-lg hover:bg-[#D4AF37] hover:text-white transition-colors">
              İncele
            </span>
        </div>
      </div>
